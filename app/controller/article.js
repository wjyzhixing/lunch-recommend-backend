...
class Article extends egg_1.Controller {
  ...
  async addProject() {
    ...
    const query = {
      ...
      name:
        (_b = ctx.request.body) === null || _b === void 0 ? void 0 : _b.name,
    };
    console.log(query);
  ...
  async updateProject() {
    ...
    const query = {
      ...
      name:
        (_c = ctx.request.body) === null || _c === void 0 ? void 0 : _c.name,
    };
    console.log(query);
    const res = await ctx.service.article.updateProject(query); // 返回值显示
    ctx.body = res;
  }
}
